...
char *
get_ip_str(const struct sockaddr *sa, char *ip_str, size_t maxlen)
{
    switch (sa->sa_family)
    {
    case AF_INET:
 ...
 */
void set_desock_localipv4(void)
{
    char *p_tmp;
 ...
 */
void set_desock_remoteipv4(void)
{
    char *p_tmp;
 ...
 */
void set_desock_port_local(void)
{
...
/* Read user defined port number to desock. Whitelist.
 */
void set_desock_port_remote(void)
{
#define MAX_PORT_LEN 6 // 65535, len is 5 + 1 (for null)
    // Server port number we want to hook.
    char port[MAX_PORT_LEN] = "";
