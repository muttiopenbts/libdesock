 ...
 */
void get_random(int *num, int lower, int upper)
{
    srand(time(0));
...
visible int accept(int fd, struct sockaddr *restrict addr, socklen_t *restrict len)
{
    ...
    DEBUG_LOG("[%s:%d:%d] Start (%d, %p, %d).\n", __FUNCTION__, __LINE__, gettid(), fd, addr, len);

    new_fd = internal_accept(fd, addr, len, 0);

    DEBUG_LOG("[%s:%d] End fd: %d, new_fd: %d.\n", __FUNCTION__, gettid(), fd, new_fd);
    return new_fd;
}
